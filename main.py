...
async def handle_praisa_command(inPuTMsG: str, sender_uid: int, chat_id: int, chat_type: int,
    ...
    name = parts[1].strip()
    messages = [
        "🌟 {Name} you are truly an amazing person!",
        "🔥 {Name} your hard work will bring you great success one day!",
        "💎 {Name} you are unique, there is no one like you!",
        "🚀 {Name} your future is very bright!",
        "👑 {Name} you are worthy of being a leader!",
        "🌈 {Name} your smile makes everyone's day better!",
        "💖 {Name} always stay this positive!",
        "🏆 {Name} you have the power to achieve whatever you want!",
        "✨ {Name} you are a source of inspiration!",
        "🌟 {Name} believe in yourself, you can do it!",
        "🎯 {Name} your focus is your strength!",
        "📈 {Name} you are getting better every day!",
        "🧠 {Name} your thinking is truly admirable!",
        "💫 {Name} you will go very far, God willing!",
        "🌍 {Name} the world is waiting to see your talent!",
        "🛡️ {Name} you are strong, confident, and brave!",
        "🏅 {Name} you are a true champion!"
    ]
...
async def handle_greeting(inPuTMsG: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    ...
    if inPuTMsG.strip().lower() in trigger_words:
        ...
        reply = "[B][C][00FF00]hii unfairarmy kaise ho sab thoda support karate raho!!"
...
async def start_match_spam_loop(uid, chat_id, chat_type, key, iv, region):
    ...
    try:
        ...
        start_msg = f"[B][C][00FF00]✅ 17 second match start spam is starting...\n🎯 Target: {uid}"
        ...
        # Completion message
        if not stop_auto:
            done_msg = "[B][C][FFFF00]⌛ 17 seconds completed! Start spam stopped."
            await safe_send_message(chat_type, done_msg, uid, chat_id, key, iv)

    except Exception as e:
        print(f"[ERROR] Spam Loop Error: {e}")
        error_msg = f"[B][C][FF0000]❌ An error occurred while spamming: {str(e)}"
...
async def send_love_with_emotes(name: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    ...
    messages = [
        ...
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF] can you give me a little [00FFFF]space in your heart, dear [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF]I have an [00FFFF]ocean-sized desire for you [FFFFFF]♡",                  
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF] you are my [00FFFF]favorite person, and I really want to be close to you [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF] you are my hope, [00FFFF]you are my love [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [00FF00]In the lobby, [FFFFFF]I only look for [FFD700]I only look for you, {Name} [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF] I can bet everything for you; [00FFFF]for you, I would cross seven seas and thirteen rivers[FFFFFF]♡",
        "[B][C][FFFFFF]♡ [00FFFF]You are [FF1493]more precious than an airdrop [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FFD700]{Name} your smile [FFFFFF]increases my [00FF00]HP [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF00FF]{Name} without you, [FFFFFF]playing the game is [00FFFF]completely pointless [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF]without you, [00FFFF]I am all alone [FFFFFF]♡",
        "[B][C][FFFFFF]♡ The sniper's [FFA500]only [FF1493]target is {Name} you [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF1493]{Name} [FFFFFF]I will build a small home with you [00FFFF][FFFFFF]♡",
        "[B][C][FFFFFF]♡ [32CD32]{Name} you are my [FFFFFF]gloo wall [FFD700]cover [FFFFFF]♡",
        "[B][C][FFFFFF]♡ [FF0000]{Name} let's [FFFFFF]be together and [00FFFF]settle down [FFFFFF]♡"
    ]
...
async def handle_gop_command(inPuTMsG: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    ...
    if name.lower() in [n.lower() for n in BLOCKED_NAMES]:
        error_msg = (
            "[B][C][FF0000]⚠️ unfair 🤣unfair 🤐unfair🥴\n[FFFFFF]unfair 🤣you🤣unfair🥴unfair\n[FF0000]sha🤣unfair🤣unfair😁d"
        )
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
...
async def handle_commands(cmd_text, sender_uid, chat_id, chat_type, key, iv, region):
    ...
    # Add this after your current /lw command
...
emote_hijack = False
BLOCK_EMOTE = False   # True = player emotes send a fixed emote to block player emotesBLOCK_EMOTE = False   # True = player emotes send a fixed emote to block player emotes
...
# FUCK YOU BRADAR ( unfair x64 defin i fuck u bruh )
...
async def send_full_player_info(data, chat_type, uid, chat_id, key, iv):
    ...
    social = data.get("socialinfo", {})  # ← use this variable
    captain = data.get("captainBasicInfo", {})
...
def load_credentials_from_file(filename="UNFAIR.txt"):
    """
    Load UID and password from UNFAIR.txt file
    """
    try:
        if not os.path.exists(filename):
...
async def handle_rizak_on_leave(uid, chat_id, key, iv, chat_type):
    ...
    try:
        ...
        if rizak_packet and online_writer:
            ...
            print(f"✅ unfairBOT packet sent on team leave/kick for UID: {uid}")

            # Send notification message
            notify_msg = "[B][C][00FF00]🔄 unfairBOT packet activated!\n[FFFFFF]Bot left team or was kicked."
...
📱 Player will see:
"unfairBOT FF  wants to join your team"
...
Possible reasons:
1. It seems the player went offline
2. Otherwise, the player is busy 
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]unfairBOT FF [00FF00]!!
"""
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]unfair[FFFF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @UNFAIR_hi\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
async def base_to_hex(value):
    ...
    # If the length is odd, add 0 at the front
    if len(hex_val) % 2 != 0:
        hex_val = '0' + hex_val
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="ALPHA"):
    ...
    try:
        ...
        nested_details.Clan_Name = "unfairBOT EMPIRE"  # CHANGE: Your clan
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="unfairBOT-FF", title_id=title_id)
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg =  f"""[B][C][FFFF00] You are calling unfairBOT a noob; this is an abusive message. 
...
"""
    ...
    try:
        ...
        # Completion message
        completion_msg =  f"""[B][C][FFFF00] Stop using these abusive titles and insults.
"""
        await safe_send_message(chat_type, completion_msg, uid, chat_id, key, iv)
                    ...
                    try:
                        ...
                        if True:
                            ...
                            try:
                                ...
                                bot_uid = 7496075460
                                message_text = f" Can't accept Your request Talk to unfairBOTFF"
...
[C][B][00FFFF]│[00FFFF]⃢▸[00FFFF]│[00FFFF]║[FFFFFF]⚡UNFAIR_YT_44⚡[00FFFF]║
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by APPLE GAMING FF 
/bio Level 70 | Pro Player
/bio Add me: APPLE GAMING FF 
...
"""
                                ...
                                credentials = load_credentials_from_file("UNFAIR.txt")
                                if not credentials:
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "7496075460":
...
"""
                            ...
                            if str(response.Data.uid) != "7496075460":  # Replace with your actual UID
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "7496075460":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "7496075460":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {uid}"
                                    count += 1
                                    if count >= 20:
...
"""
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[B][C][FF0000] Looking to buy a bot at a low price?

Want to buy a bot in your own name?

Contact me

[B][C][FF0000]TELEGRAM CHANNEL: [FFFFFF]t.me/UNFAIR_api

[B][C][FF0000]TELEGRAM GROUP: [FFFFFF]https://t.me/unfairyt_ff
"""
...
"""
                        ...
                        if inPuTMsG.startswith('/exit'):
                            ...
                            # SUCCESS MESSAGE
                            success_message = f"[B][C][FFFF00]✅ Why did you remove the BOT, bro? G👽A👽Y\n"
                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
...
[B][00FFFF]Example: /ghost 4337272 unfairBOT
"""
...
[B][FFFFFF]━━━━━━━━━━━━━━━━━━
[B][FF69B4]unfaireloper : unfairBOT
"""
                                    else:
                                        ghost_msg = f"""
[B][FF0000]❌ API FAILED
...
"""
                                ...
                                try:
                                    ...
                                    r = requests.get(url, timeout=10)
                                    data = r.text  # Treat as response string
...
#                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            ...
                            initial_message = f'[B][C]{get_random_color()}\n🎭 Poor people use BOT to send Emotes.....\n'
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
    ...
    "jisan123", "unfairBOT1", "jisan", "jisan@123", "Jisan1", "jisan@1", 
    ...
    "JISAN_FayEz", "@fayez_jisan", "FAYEZ", "Fayez", 
    "fayezjisan", "JISAN", "@Jisan_FayEz", "Fayez", 
    "FUHX", "jisan.", "fayez ullah hawlader", "FUH X", "Fayez" ]
                            ...
                            try:
                                ...
                                if len(parts) < 2:
                                    ...
                                    messages = [
                                        ...
                                        "{Name} [B][C][00FFFF] TMC BSDK MC BC BLK BUR LAND TERI MA KI CHHUT!",
                                        "{Name} [B][C][00FFFF]  TMC BSDK MC BC BLK BUR LAND TERI MA KI CHHUT!",
                                        "{Name} [B][C][00FFFF]I TMC BSDK MC BC BLK BUR LAND TERI MA KI CHHUT।!!",

                                        "{Name} [B][C][00FFFF] TMC BSDK MC BC BLK BUR LAND TERI MA KI CHHUT!",
                                        "{Name} [B][C][00FFFF] One TMC BSDK MC BC BLK BUR LAND TERI MA KI CHHUT!!",
                                        "{Name} [B][C][00FFFF] Two TMC BSDK MC BC BLK BUR LAND TERI MA KI CHHUT!!"
                                            ]
...
# ==================== /luck COMMAND ====================
                        if inPuTMsG.strip() == "/luck":
                            luck_messages = [
                                "Today, if you go social, you can make Mia Khalifa your GF ....!! ⚡",
                                "Today, C2k will drop in your match 🔥",
                                "Today, when you go social, you will find a girl with 4 holes 100%🏆",
                                "Today you can have fun with friends.....come and see..!! 😎",
                                "Some things will go well by luck today...!! For example, you may meet someone when joining 🍀"
                            ]
                            selected_message = random.choice(luck_messages)
                            admin_message = (
                                f"[C][B][00FFFF]╔════════════════════╗\n"
                                f"║  ╭──────────────╮\n"
                                f"║  │[FF0000] Luck Testing Center         │\n"
                                f"║  ╰──────────────╯\n"
                                f"║\n"
                                f"║ [FFFFFF]✨ {selected_message}\n"
                                f"║ ────────────────\n"
                                f"║ ⚡ [FFFF00] unfairBOT FF ⚡\n"
                                f"╚════════════════════╝"
                            )
                            await safe_send_message(response.Data.chat_type, admin_message, uid, chat_id, key, iv)
...
# ==================== /coin COMMAND ====================
                        if inPuTMsG.strip().startswith('/coin'):
                            ...
                            coin_message = (
                                ...
                                f"║ 🙋 Flipped by: [FFFF00] unfairBOT FF  \n"
                                f"║ ▪ Try again: /coin\n"
                                f"╚════════════════════╝"
                            )
...
# ==================== /dhadha COMMAND ====================
                        if inPuTMsG.strip() == "/dhadha":
                            dha_messages = [
                                "Everyone looks for me before entering the game, but when an enemy appears, everyone blames me! Who am I? ",
                                "Sometimes I am red, sometimes blue; when you enter me, your body becomes loose! Who am I? ",
                                "I look small, but I contain lots of loot! Everyone runs to me when they see me. Who am I? ",
                                "Everyone wants to kill me, but everyone is happy when I die. Who am I?', 'a': 'A lobby enemy or bot! 🤖",
                                "'I have no eyes but I see, no legs but I move. My only job is to catch your mistakes! Who am I? "
                                "' Three people hold one while another attacks "
                            ]
                            selected_message = random.choice(dha_messages)
                            dhadha_message = (
                                f"[C][B][00FFFF]╔══════════╗\n"
                                f"║  ╭────────────╮\n"
                                f"║  │[FF69B4] unfairNEW BOT OB54 🤔       │\n"
                                f"║  ╰───────────╯\n"
                                f"║\n"
                                f"║ [FFFFFF]✨ {selected_message}\n"
                                f"║ ────────────\n"
                                f"║ ⚡ [FF00FF] unfairBOT FF BOT⚡\n"
                                f"╚════════════╝"
                            )
...
#GALI SPAM MESSAGE 
    ...
    # Add your actual name
                        ...
                        # =================== COMPLETE HELP MENU (SEPARATE PARTS) ===================
                         ...
                         # =================== WORKING COMMANDS HELP MENU ===================
                        if inPuTMsG.strip().lower() in ("help", "/help", "halp", "UNFAIR", "***"):
                            print(f"Help command detected from UID: {uid} in chat type: {XX}")
                            try:
                                basic = """[B][C][DC143C]╱╲╱╲╱╲╱╲╱╲
[C][B][FFD700]   👑 unfairBOT FF 👑
[C][B][00FFFF]    BLADE SYSTEM
[C][B][DC143C]╲╱╲╱╲╱╲╱╲╱

[39FF14]🂱 INVITE•ᴗ•PLAYER𓇢𓆸
[DC143C]➺ /inv

[39FF14]➺ 3 PLAYER•ᴗ•GROUP𓇢𓆸
[DC143C]↬ /3

[39FF14]🂱 4 PLAYER•ᴗ•GROUP𓇢𓆸
[DC143C]╰•➤ /4

[39FF14]➺ 5 PLAYER•ᴗ•GROUP𓇢𓆸
[DC143C]⪼------➢ /5

[39FF14]➺ 6 PLAYER•ᴗ•GROUP𓇢𓆸
[DC143C]↬ /6

[DC143C]╱╲ UNFAIR•ᴗ•KING𓇢𓆸 ╲╱"""
                                await safe_send_message(response.Data.chat_type, basic, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                emotes = emotes = """[B][C][7B2EFF]◤◢◤◢◤◢◤◢◤◢◤◢◤◢
[C][B][00F5FF]      ⚡ unfairBOT FF ⚡
[C][B][39FF14]       CYBER PANEL
[C][B][7B2EFF]◣◥◣◥◣◥◣◥◣◥◣◥◣◥

[FFB300]🂱 SPAM INVITE𓇢𓆸
[00F5FF]╰┈> /spam_group [uid]𓇢𓆸
[FFB300]◤ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ

[FF006E]🂱 JOIN BY UID𓇢𓆸
[00F5FF]╰┈> /join_req [uid]
[FF006E]◤ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ

[00E676]🂱 JOIN BY CODE𓇢𓆸
[00F5FF]╰┈> /join [tc]
[00E676]◤ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ

[FF6D00]🂱 LEAVE GROUP𓇢𓆸
[00F5FF]╰┈> /exit
[FF6D00]◤ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ

[8E24AA]🂱 FAST EMOTE𓇢𓆸
[00F5FF]╰┈> 2×200
[8E24AA]◤ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ

[B][C][7B2EFF]◢━━━━━━━━━━━━━━━━━━━━◣
[C][B][FFD700]     ★ ELITE ACCESS ★𓇢𓆸
[C][B][7B2EFF]◥━━━━━━━━━━━━━━━━━━━━◤"""
                                await safe_send_message(response.Data.chat_type, emotes, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                evo = """[B][C][6A00FF]╭━━━◤ ⚜ ◥━━━╮
[C][B][00F5D4]      👑 UNFAIR_YT_44 𓇢𓆸👑
[C][B][FFD700]      ✦ ROYAL CORE 𓇢𓆸✦
[B][C][6A00FF]╰━━━◣ ⚜ ◢━━━╯

[FF4D6D]➺ AUTO EVO EMOTE𓇢𓆸
[00F5D4]╰➤ @evos
[FF4D6D]· · ─ ·𖥸· ─ · ·

[00E676]➺ OFF EVO EMOTE𓇢𓆸
[FFD700]╰➤ @sevos
[00E676]· · ─ ·𖥸· ─ · ·

[2979FF]➺ EVO TO PLAYER𓇢𓆸
[00F5D4]╰➤ /evo [uid] [num]
[2979FF]· · ─ ·𖥸· ─ · ·
...
[E91E63]➺ NUMBER TO EMOTE𓇢𓆸
[00F5D4]➳ [1-4]10

[B][C][6A00FF]◢ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ◣
[C][B][FFD700]    ★ unfairKING 𓇢𓆸 ★
[B][C][6A00FF]◥ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩ٨ـ◤"""
                                await safe_send_message(response.Data.chat_type, evo, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                fun_cmnd = """[B][C][00C853]╔════◈════╗
[C][B][FF1744]      UNFAIR_YT_44𓇢𓆸
[C][B][00E5FF]    CYBER X PANEL𓇢𓆸
[B][C][00C853]╚════◈════╝

[7C4DFF]—͟͟͞͞★ START MATCH𓇢𓆸
[FFD700]⪼------➢ /start1
[7C4DFF]════════════

[FF6D00]—͟͟͞͞★ DUO CHECK𓇢𓆸
[00E5FF]⪼------➢ /duo [uid]
[FF6D00]────◆────

[00E676]—͟͟͞͞★ RESPECT𓇢𓆸
[FFD700]⪼------➢ /praisa [name]
[00E676]────◆────

[E91E63]—͟͟͞͞★ LONEWOLF𓇢𓆸
[00E5FF]⪼------➢ /lw [tc]
[E91E63]─•──── 𖦤
...
[B][C][00C853]╔════════════╗
[C][B][FFD700]  —͟͟͞͞★ unfairKING𓇢𓆸 ◆
[B][C][00C853]╚════════════╝"""
                                await safe_send_message(response.Data.chat_type, fun_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                spam_cmnd = """[B][C][FF1744]◤═══════◥
[C][B][00F5FF]     UNFAIR_YT_44𓇢𓆸
[C][B][FFD700]    MAGIC PANEL𓇢𓆸
[B][C][FF1744]◣═══════◢

[7B1FA2]♡ MAGIC COLOUR𓇢𓆸
[00F5FF]↳ /magic
[7B1FA2]─•──── 𖦤

[00C853]♡ CALL ME NOOB𓇢𓆸
[FFD700]↳ /noob
[00C853]─•──── 𖦤
...
[B][C][FF1744]◤════════════◥
[C][B][FFD700]  ♡ VIP MAGIC𓇢𓆸 ♡
[B][C][FF1744]◣════════════◢"""
                                await safe_send_message(response.Data.chat_type, spam_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                badge_cmnd = """[B][C][1565C0]╭═════⬢═════╮
[C][B][39FF14]     UNFAIR_YT_44𓇢𓆸
[C][B][FFD700]    BADGE HUB𓇢𓆸
[B][C][1565C0]╰═════⬢═════╯
...
[B][C][1565C0]─•─ 𖦤 VIP𓇢𓆸 ─•── 𖦤"""
                                await safe_send_message(response.Data.chat_type, badge_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                ad_cmnd = """[B][C][8E24AA]╔══✦══╗
[C][B][00F5FF]   UNFAIR_YT_44𓇢𓆸
[C][B][FFD700]   TEXT ZONE𓇢𓆸
[B][C][8E24AA]╚══✦══╝

[FF1744]◆ NORMAL GALI𓇢𓆸
[00F5FF]➜ /gali [name]
[FF1744]⫘⫘⫘⫘⫘⫘

[00E676]◆ DANGER GALI𓇢𓆸
[FFD700]➜ /gali [name]
[00E676]⫘⫘⫘⫘⫘⫘
...
[2979FF]◆ BUNDLE AUTO𓇢𓆸
[00F5FF]➜ /magicbundle [tc]

[B][C][8E24AA]⫘⫘ UNFAIR_YT_44𓇢𓆸 ⫘⫘"""
                                await safe_send_message(response.Data.chat_type, ad_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                info_cmnd = """[B][C][00B8D4]◥✦•┈๑⋅⋯⋯⋅๑┈·✦◤
[C][B][FFEA00]      UNFAIR_YT_44𓇢𓆸
[C][B][FF4081]    SHADOW MENU𓇢𓆸
[B][C][00B8D4]◣✦•┈๑⋅⋯⋯⋅๑┈·✦◢

[7C4DFF]❀ OFFLINE TEAM𓇢𓆸
[00FFFF]➜ /offline [tc]
[7C4DFF]✦•┈๑⋅⋯⋯⋅๑┈·✦

[00E676]❀ EVO BUNDLE LIST𓇢𓆸
[FFD700]➜ /b
[00E676]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ

[FF6D00]❀ EVO ANIMATION𓇢𓆸
[00FFFF]➜ /animation
[FF6D00]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ

[E91E63]❀ EVO COMBO𓇢𓆸
[FFD700]➜ /look

[B][C][00B8D4]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩunfairKINGﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ"""
                                await safe_send_message(response.Data.chat_type, info_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                about = """[B][C][D500F9]╭━━━◆━━━╮
[C][B][39FF14]     UNFAIR_YT_44𓇢𓆸
[C][B][FFD700]   ROOM SYSTEM𓇢𓆸
[B][C][D500F9]╰━━━◆━━━╯

[00E5FF]❀ EMOTE BLOCK𓇢𓆸
[FF1744]➜ /block_emote [uid]
[00E5FF]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ

[FF9100]❀ EMOTE UNBLOCK𓇢𓆸
[39FF14]➜ /unblock_emote [uid]
[FF9100]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ

[2979FF]❀ PLAYER INFO𓇢𓆸
[FFD700]➜ /status [uid]
[2979FF]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ

[E91E63]❀ INVITE LAG𓇢𓆸
[00FFFF]➜ /UNFAIR lag [uid]
[E91E63]ﮩ٨ـﮩﮩ٨ـ♡ﮩ٨ـﮩﮩﮩ٨ـ

[7C4DFF]❀ JOIN ROOM𓇢𓆸
[FFD700]➜ /xjoin [room_id] [pass]

[B][C][D500F9]══ ELITE ACCESS𓇢𓆸 ══"""
                                await safe_send_message(response.Data.chat_type, about, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                whitelist_cmnd = """[B][C][FF3D00]◤⋆──⃝─⃝⃝⃝──⋆◥
[C][B][00E5FF]     UNFAIR𓇢𓆸
[C][B][39FF14]    GAME PANEL𓇢𓆸
[B][C][FF3D00]◣⋆──⃝─⃝⃝⃝──⋆◢
...
[2979FF]❀ UID INFO𓇢𓆸
[FFD700]➜ /info [uid]
[2979FF]ـــــــــــــــــــــــﮩ٨ـ❤️️

[E91E63]❀ ADMIN INFO𓇢𓆸
[00FFFF]➜ /admin
[E91E63]ـــــــــــــــــــــــﮩ٨ـ❤️️
[00C853]❀ KICK PLAYER𓇢𓆸
[FFD700]➜ /kick [uid]
[00C853]ـــــــــــــــــــــــﮩ٨ـ❤️️

[FF6D00]❀ NOTE𓇢𓆸
[00FFFF]➜ BOT MUST BE LEADER

[B][C][FF3D00]════ GAME VIP𓇢𓆸 ════"""
                                await safe_send_message(response.Data.chat_type, whitelist_cmnd, uid, chat_id, key, iv)
                                await asyncio.sleep(0.2)

                                last = """[B][C][1E88E5]╔═══════◈═══════╗
[C][B][FFD700]     UNFAIR_YT_44𓇢𓆸
[C][B][00FF99]      VIP PANEL𓇢𓆸
[B][C][1E88E5]╚═══════◈═══════╝

[FF1744]❀ GENDER CHECK𓇢𓆸
[00FFFF]➜ /gender [name]
[FF1744]ـــــــــــــــــــــــﮩ٨ـ❤️️

[8E24AA]❀ NO-BOT EMOTE𓇢𓆸
[39FF14]➜ /em [tc] [num]
[8E24AA]ـــــــــــــــــــــﮩ٨ـ❤️️

[FF9100]❀ GHOST JOIN𓇢𓆸
[FFD700]➜ /ghost [tc] [name]
[FF9100]⫘⫘⫘⫘⫘⫘
...
[E91E63]❀ 5 PLAYER SQUAD𓇢𓆸
[39FF14]➜ /snd [uid]

[B][C][1E88E5]⫘⫘unfairKING ⫘⫘"""
                        ...
                        # =================== unfairBOT NAME DETECTION ===================
                        # Detect variations of unfairBOT in message (case-insensitive)
                        if "/unfairBOTff" in original_msg.lower():
                            print("unfairBOT name detected! Sending colored ASCII art...")
...
"""
                        ...
                        if inPuTMsG.strip().isdigit() or (inPuTMsG.strip().count('*') == 1 and inPuTMsG.strip().split('*')[0].isdigit() and inPuTMsG.strip().split('*')[1].isdigit()):
                            ...
                            if emote_id:
                                # Send initial message
                                if times == 1:
                                    msg = f"[B][C][FFFFFF] I..LOVE..YOU..Sadia"
                                else:
                                    msg = f"[B][C][FFFFFF] I..LOVE..YOU..Sadia"
                                await safe_send_message(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                ...
                                success_msg = f"[B][C][FFFFFF] I..LOVE..YOU..Sadia"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                        ...
                        if original_msg and "StickerStr" in original_msg:
                            ...
                            if NUMBER_EMOTES:
                                ...
                                try:
                                    ...
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H)
                                    # Also send a message
                                    sticker_reply = "[B][C][FFFF00] unfairBOT STYLE TCP"
...
async def handle_addfriend_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 2:
        ...
        return
    target_uid = parts[1]
    # Load credentials from UNFAIR.txt
    credentials = load_credentials_from_file("UNFAIR.txt")
    if not credentials or len(credentials) < 2:
        error_msg = f"[B][C][FF0000]❌ Failed to load credentials from UNFAIR.txt\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
...
# =================== NEW FUNCTION: /unfairBOTlag LOOP (fixed: no leave at end, added small delays) ===================
chatspam_running = False
chatspam_task = None
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from UNFAIR.txt...")
    credentials = load_credentials_from_file("UNFAIR.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create UNFAIR.txt with your UID and password")
...
""")
    print("=" * 50)
    print("🤖 unfairYT BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 unfairYT TCP BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 unfairYT TCP BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
...
""")
    print("=" * 50)
    print("=" * 50)
    print("🤖 unfairunfairYT TCP BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
